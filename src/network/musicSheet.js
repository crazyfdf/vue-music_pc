...
export class SongData {
  constructor(songs) {
    this.id = songs[0].id; //音乐id
    this.name = songs[0].name; //歌名
    this.album = songs[0].al.name; //专辑名
    this.song = songs[0].ar[0].name; //用户名
    this.picUrl = songs[0].al.picUrl; //用户头像
    this.time = formatDate(new Date(songs[0].dt), 'mm:ss') //音乐时间
  }
}
//歌单数据
